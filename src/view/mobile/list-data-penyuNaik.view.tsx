...
const TukikList = ({ Track = false,Button=false }: { Track?: boolean,Button?:boolean }) => {
  ...
  useEffect(() => {
    if (data?.data) {
      const groupedData = groupByPenyuBertelur(data.data);
   ...
   return (
    <Suspense>
      <div className={"p-4 md:p-8"}>
        {!Track && (
          <Breadcrumb className="mb-4">
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>{penyuNaik.tittle}</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        )}
        ...
        {showEmptyState && (
          <EmptyView
            Title={`Tidak Ada ${penyuNaik.tittle}`}
            CardDescription={penyuNaik.descriptionNull}
              ...
              <Grid item xs={4} sm={4} md={4} key={item.id}>
                <CardComponent
                  ...
                  item={{
                    ...
                    status: !Track? undefined : item.penyuBertelur ? "Bertelur" : "Tidak Bertelur",
                  }}
                />
              </Grid>
            ))}
