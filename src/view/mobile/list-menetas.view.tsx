...
const TukikList = ({ Track = false }: { Track?: boolean }) => {
    ...
    const [Selected, setSelected] = useState<Menetas | null>(null);
    ...
    const handleDateChange = (date: Date) => {
      const filtered = initialData.filter((item: Menetas) => {
    ...
    const onDeleteOpen = useCallback((dataPenyu: Menetas) => {
      setSelected(dataPenyu);
      setIsDialogOpen(true);
      setIsDeleted(true);
    }, []);
  
    const onEditOpen = useCallback((dataPenyu: Menetas) => {
      setSelected(dataPenyu);
      setIsDialogOpen(true);
    }, []);

    const onViewOpen = useCallback((dataPenyu: Menetas) => {
      setSelected(dataPenyu);
      setIsDialogOpen(true);
      setIsView(true);
    }, []);
          ...
          {!Track && (
            <Breadcrumb className="mb-4">
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbPage>Menetas</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          )}
                ...
                <Grid item xs={4} sm={4} md={4} key={item.id}>
                  <CardComponent
                    ...
                    item={{
                      ...
                      jumlah: `Jumlah Menetas ${item.telurMenetas}`,
                      status: !Track?undefined:item.diinkubasi ? "Menetas" : "Dirawat"  ,
                    }}    
                  />
                </Grid>
              ))}
